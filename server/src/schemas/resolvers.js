...
const resolvers = {
  ...
  Mutation: {
    ...
    addBook: async (_, { title, author, publishedDate }) => {
      return await Book.create({ title, author, publishedDate });
    },
    addReview: async (_, { bookId, reviewer, rating, comment }) => {
      return await Review.create({ bookId, reviewer, rating, comment });
    },
